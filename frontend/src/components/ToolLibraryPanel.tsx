...
export default function ToolLibraryPanel({
  ...
  const emailTools = ["email_send", "email_read"];
  const builtInTools = catalog.filter((t) => !emailTools.includes(t.id) && t.category !== "mcp");
  const emailToolEntries = catalog.filter((t) => emailTools.includes(t.id));
  const mcpTools = catalog.filter((t) => t.category === "mcp");
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => handleBatchToggle(builtInTools, true)}
                      disabled={builtInTools.every((t) => t.in_library)}
                    ...
                    <button
                      onClick={() => handleBatchToggle(builtInTools, false)}
                      disabled={builtInTools.every((t) => !t.in_library)}
                      className="px-2 py-0.5 text-[10px] rounded bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      Disable all
