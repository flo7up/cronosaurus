"""
OpenAI provider — uses Microsoft Agent Framework with OpenAIChatClient.
...
# ── Thread-local event queue for SSE tool call events ─────────
...
def stream_response(
    *,
    ...
    """Stream a response using Agent Framework's OpenAIChatClient."""
    api_key = settings.openai_api_key
    if not api_key:
        yield json.dumps({"type": "error", "content": "OpenAI API key not configured. Set it in Settings."})
        return

    model_name = model or settings.openai_model
    event_q: queue.Queue = queue.Queue()

    # Refs so tool wrappers can access current context
    agent_id_ref = [agent_id]
    thread_id_ref = [thread_id]
    model_ref = [model]

    # Create AF tool wrappers for each tool definition
    af_tools = []
    for td in tool_defs:
        try:
    ...
    async def _run_agent():
        ...
        try:
            client = OpenAIChatClient(api_key=api_key, model_id=model_name)
            ...
            logger.error("AF OpenAI agent error: %s", e, exc_info=True)
            error_msg = str(e)
        finally:
            event_q.put(None)  # sentinel

    # Run the async agent in a background thread
    def _thread_target():
    ...
    if error_msg:
        yield json.dumps({"type": "error", "content": f"OpenAI error: {error_msg}"})
    ...
    yield json.dumps({"type": "done", "content": full_response})
        ...
        # Continue the loop — the model will generate a response based on tool results

    # Exhausted rounds
    yield json.dumps({"type": "error", "content": "Too many tool call rounds. Try a simpler request."})
